...
*/
select customer_id,count(r.id) from bookings b
join reserved_seat r on b.id=r.booking_id
join screenings s on s.id= b.screening_id
group by customer_id,start_time
having start_time between '2017-10-01' and '2017-10-31';

select name,count(s.id) from films f
...
select name,max(s.id),min(s.id) from screenings s
